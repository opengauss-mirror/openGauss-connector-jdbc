...
public class ORQueryExecutorImpl implements ORQueryExecutor {
    ...
    private void getResult(ORCachedQuery cachedQuery, int remainLen)
        ...
        if (packageHead.getExecResult() != 0) {
            ...
            if (packageHead.getExecCmd() == ORRequestCommand.FETCH) {
                ...
                orStream.receiveChar();
                int rowRemainLen = remainLen - 8;
                if (total > 0) {
                    List<int[]> valueLens = new ArrayList<>();
                    List<byte[][]> rows = new ArrayList<>();
                    this.getByteData(valueLens, rows, total, rowRemainLen);
                ...
                if (queryMode == DCL_SQL || queryMode == DDL_SQL) {
                    ...
                    this.handleResult(cachedQuery, cachedQuery.getStatement().getField(), remainLen);
                }
            } else if (packageHead.getExecCmd() == ORRequestCommand.READ_LOB_DATA) {
                handleLobData(cachedQuery, remainLen);
            } else {
                handleData(cachedQuery, remainLen);
            }
        }
    }

    private void handleLobData(ORCachedQuery cachedQuery, int remainLen)
            throws SQLException, IOException {
        if (remainLen < 8) {
            throw new PSQLException("LOB data length error.", PSQLState.PROTOCOL_VIOLATION);
        }
        int dataLen = orStream.receiveInteger4();
        int hasData = orStream.receiveInteger4();
        if (dataLen < 0 || dataLen > remainLen - 8) {
            throw new PSQLException("LOB data length error.", PSQLState.PROTOCOL_VIOLATION);
        }
        if (dataLen == 0) {
    ...
    private void handleData(ORCachedQuery cachedQuery, int remainLen) throws SQLException, IOException {
        ...
        ensureRemainingBytes(remainLen, cols, "field count");
        ...
        if (queryMode == DCL_SQL || queryMode == DDL_SQL) {
            ...
            handleResult(cachedQuery, fields, remainLen);
        }
    }
    ...
    private void handleResult(ORCachedQuery cachedQuery, ORField[] fields, int remainLen)
            throws SQLException, IOException {
        ...
        int bytesRemain = remainLen - 8;
        if (fields.length > 0) {
            List<int[]> valueLens = new ArrayList<>();
            List<byte[][]> rows = new ArrayList<>();
            if (total > 0) {
                bytesRemain -= this.getByteData(valueLens, rows, total, bytesRemain);
            }
            cachedQuery.handleResultRows(fields, valueLens, rows, hasRemain);
        }
        if (cachedQuery.isAutoGeneratedKeys() && total > 0) {
            handleGeneratedKeys(cachedQuery, total, hasRemain, bytesRemain);
        }
    }

    private void handleGeneratedKeys(ORCachedQuery cachedQuery, int total, boolean hasRemain,
            int remainLen) throws IOException, SQLException {
        ...
        if (queryFlag == INSERT_SQL || queryFlag == MERGE_SQL) {
            this.getByteData(valueLens, generatedKeysRows, total, remainLen);
        }
        cachedQuery.handleGeneratedKeys(fields, valueLens, generatedKeysRows, hasRemain);
    }

    private int getByteData(List<int[]> valueLens, List<byte[][]> rows, int total, int remainLen)
            throws SQLException, IOException {
        int consumed = 0;
        int bytesRemain = remainLen;
        for (int i = 0; i < total; i++) {
            int rowLen = getRow(valueLens, rows, bytesRemain);
            consumed += rowLen;
            bytesRemain -= rowLen;
        }
        return consumed;
    ...
    private int getRow(List<int[]> valueLens, List<byte[][]> rows, int remainLen)
            throws SQLException, IOException {
        ensureRemainingBytes(remainLen, 4, "row header");
        orStream.receiveInteger2();
        int columns = orStream.receiveInteger2();
        ensureRemainingBytes(remainLen - 4, columns, "row column count");
        ...
        int consumed = 4;
        ensureRemainingBytes(remainLen - consumed, 1 + lenOperation, "row metadata");
        orStream.receiveChar();
        byte[] lenMark = orStream.receive(lenOperation);
        consumed += 1 + lenOperation;
        int index = 0;
        int colIndex = 0;
        while (true) {
            int p = index * 4;
            ...
            for (int k = p; k < p + 4; k++) {
                ...
                int lenId = mark & 3;
                consumed += handleValue(lenId, lens, value, k, remainLen - consumed);
                mark = mark >> 2;
                colIndex = k;
            }
    ...
    private int handleValue(int lenId, int[] valueLen, byte[][] value, int k, int remainLen)
            throws SQLException, IOException {
        if (lenId == BYTE0_SIGN) {
            valueLen[k] = -1;
            value[k] = new byte[0];
            ...
            ensureRemainingBytes(remainLen, 4, "row value");
            valueLen[k] = 4;
            value[k] = orStream.receive(4);
            return 4;
        } else if (lenId == BYTE8_SIGN) {
            ensureRemainingBytes(remainLen, 8, "row value");
            valueLen[k] = 8;
            value[k] = orStream.receive(8);
            return 8;
        } else {
            ensureRemainingBytes(remainLen, 2, "row value length");
            int len = orStream.receiveInteger2();
            valueLen[k] = len;
            int byte4Len = (4 - (len + 2) % 4) % 4;
            int totalLen = byte4Len + len;
            ensureRemainingBytes(remainLen - 2, totalLen, "row value");
            value[k] = orStream.receive(totalLen);
            return 2 + totalLen;
        }
    }

    private void ensureRemainingBytes(int remainLen, int needed, String message) throws SQLException {
        if (needed < 0 || needed > remainLen) {
            throw new PSQLException(message + " length error.", PSQLState.PROTOCOL_VIOLATION);
        }
    }
    ...
    private void getCursorResult(ORCursorResultSet cursorRs, int remainLen) throws IOException, SQLException {
        if (packageHead.getExecResult() != 0) {
            ...
            handleCursorRows(cursorRs, remainLen);
        }
    }

    private void handleCursorRows(ORCursorResultSet cursorRs, int remainLen)
        ...
        orStream.receiveChar();
        int rowsRemainLen = remainLen - 8;
        int totalCol = cursorRs.getStatement().getField().length;
        if (totalCol > 0) {
            List<int[]> valueLens = new ArrayList<>();
            List<byte[][]> rows = new ArrayList<>();
            if (rowsCount > 0) {
                this.getByteData(valueLens, rows, rowsCount, rowsRemainLen);
            }
